...
bool VertexBuffer::init(VkRenderData& renderData)
{
    ...
    if (vmaCreateBuffer(renderData.rdAllocator, &bufferInfo, &bufferAllocInfo, &renderData.rdVertexBuffer,
                        &renderData.rdVertexBufferAlloc, nullptr) != VK_SUCCESS)
    {
    ...
    if (vmaCreateBuffer(renderData.rdAllocator, &stagingBufferInfo, &stagingAllocInfo,
                        &renderData.rdVertexStagingBuffer, &renderData.rdVertexStagingBufferAlloc,
                        nullptr) != VK_SUCCESS)
    {
...
bool VertexBuffer::uploadData(VkRenderData& renderData, VkMesh vertexData)
{
    ...
    vkCmdCopyBuffer(renderData.rdCommandBuffer, renderData.rdVertexStagingBuffer, renderData.rdVertexBuffer, 1,
                    &stagingBufferCopy);
...
{
    vmaDestroyBuffer(renderData.rdAllocator, renderData.rdVertexStagingBuffer, renderData.rdVertexStagingBufferAlloc);
    vmaDestroyBuffer(renderData.rdAllocator, renderData.rdVertexBuffer, renderData.rdVertexBufferAlloc);
}

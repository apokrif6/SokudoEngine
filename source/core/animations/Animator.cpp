...
void Core::Animations::Animator::updateBonesTransform(Renderer::Mesh* mesh)
{
    for (Renderer::Primitive& primitive : mesh->getPrimitives())
    {
        ...
        const aiScene* scene =
            Animations::AnimatorSingleton::getInstance().importers[mesh->getMeshName()].get()->GetScene();
...
const aiNodeAnim* Core::Animations::Animator::findAnimationNode(const aiAnimation* animation,
...
glm::vec3 Core::Animations::Animator::interpolatePosition(const aiNodeAnim* nodeAnim, float animationTime)
{
...
glm::quat Core::Animations::Animator::interpolateRotation(const aiNodeAnim* nodeAnim, float animationTime)
{
...
glm::vec3 Core::Animations::Animator::interpolateScaling(const aiNodeAnim* nodeAnim, float animationTime)
{

...
bool Core::Renderer::ViewportTarget::init(VkRenderData& renderData)
{
    ...
    if (!createDescriptor(renderData)) return false;
    return true;
}
...
bool Core::Renderer::ViewportTarget::createImage(VkRenderData& renderData)
{
    ...
    if (vmaCreateImage(renderData.rdAllocator, &imageInfo, &allocInfo,
                       &renderData.rdViewportTarget.image,
                       &renderData.rdViewportTarget.imageAlloc,
                       nullptr) != VK_SUCCESS)
    {
...
bool Core::Renderer::ViewportTarget::createImageView(VkRenderData& renderData)
{
    ...
    if (vkCreateImageView(renderData.rdVkbDevice.device, &viewInfo, nullptr,
                          &renderData.rdViewportTarget.imageView) != VK_SUCCESS)
    {
...
bool Core::Renderer::ViewportTarget::createFramebuffer(VkRenderData& renderData)
{
    VkImageView attachments[] = {
            renderData.rdViewportTarget.imageView,
...
void Core::Renderer::ViewportTarget::cleanup(VkRenderData& renderData)
{
    ...
    vmaDestroyImage(renderData.rdAllocator, renderData.rdViewportTarget.image,
                    renderData.rdViewportTarget.imageAlloc);
}

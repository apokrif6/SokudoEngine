...
public:
  ...
  void addChildren(const std::vector<int>& childNodes);
  ...
  [[nodiscard]] glm::mat4 getNodeMatrix() const { return mNodeMatrix; }

  void printTree();

private:
  void printNodes(const std::shared_ptr<GltfNode>& startNode, int indent);
